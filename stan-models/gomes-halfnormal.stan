// variation of gomes model with a half-normal prior on alpha_hier instead of
// a sparsity inducing gamma prior
data {
  ...
  matrix[N2, M] f; // h * s (probability of dying on day i given infection)
...
parameters {
  ...
  vector<lower=0>[P] alpha_hier; // sudo parameter for the hier term for alpha
  real<lower=0> gamma;
  vector[M] lockdown;
...

    {
      ...
      for (m in 1:M){
        ...
        for (i in (N0+1):N2) {
          ...
          Rt_adj[i,m] = ((pop[m]-cumm_sum[i,m]) / pop[m])^(1 + 1 / a_het) * Rt[i,m];
...
generated quantities {
...

    {
      ...
      for (m in 1:M){
        ...
        for (i in (N0+1):N2) {
          ...
          prediction0[i, m] = ((pop[m]-cumm_sum0[i,m]) / pop[m])^(1 + 1 / a_het) * mu[m] * convolution0;
        }
    ...
    for (m in 1:M) {
      log_lik[m] = neg_binomial_2_lpmf(deaths[EpidemicStart[m]:N[m], m] | E_deaths[EpidemicStart[m]:N[m], m], phi);
    }
}
